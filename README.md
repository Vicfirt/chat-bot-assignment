...
`docs/eval-results.{md,json}` and `docs/loadtest-results.md` are generated by
`python -m eval.run_eval` and `python -m loadtest.run_load`. The committed run is
`LLM_MODE=ollama` / **`llama3.2:1b`** (the load test with the real model is
slow — n=30; the retrieval/functional eval covers all 15). A `3b` run can be
added the same way.
